...
const UnBlockCustomerSlicer = createSlice({
  name :"dcalabash/UnBlockCustomer",
  initialState,
  reducers: {
  },

  extraReducers: (builder) => {
    builder.addCase(UnBlockCustomerApi.pending, (state) => {
      state.isLoading = true;
    });
    builder.addCase(UnBlockCustomerApi.fulfilled, (state) => {
      state.isLoading = false;
      // console.log(localStorage.getItem("token"));
    });
    builder.addCase(UnBlockCustomerApi.rejected, (state) => {
      state.isLoading = false;
      state.isError = true;
    });
  },
...
export default UnBlockCustomerSlicer.reducer;
