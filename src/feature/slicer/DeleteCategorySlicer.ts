...
import { GetCategoriesListApi } from "./GetCategoriesSlicer";

export const DeleteCategoryApi: any = createAsyncThunk(
  "dcalabash/DeletCat",
  async (Obj: any, {dispatch}) => {
    return await axios
    .post(`${baseUrl}categories/delete-category`, Obj,config)
    .then((resp) => {
    // console.log(resp);
...
const DeleteCategorySlicer = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(DeleteCategoryApi.fulfilled, (state) => {
      state.isLoading = false;
      // console.log(localStorage.getItem("token"));
    });
    builder.addCase(DeleteCategoryApi.rejected, (state) => {
      state.isLoading = false;
      state.isError = true;
    });
  },
