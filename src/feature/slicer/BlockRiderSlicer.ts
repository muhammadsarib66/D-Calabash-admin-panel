...
const BlockRiderSlicer = createSlice({
  name :"dcalabash/BlockRider",
  initialState,
  reducers: {
  },

  extraReducers: (builder) => {
    builder.addCase(BlockRiderApi.pending, (state) => {
      state.isLoading = true;
    });
    builder.addCase(BlockRiderApi.fulfilled, (state) => {
      state.isLoading = false;
      // console.log(localStorage.getItem("token"));
    });
    builder.addCase(BlockRiderApi.rejected, (state) => {
      state.isLoading = false;
      state.isError = true;
    });
  },
...
export default BlockRiderSlicer.reducer;
